...
describe Dessert do
  let(:chef) { double("chef", name: "Chloe") }
  ...
  describe "#add_ingredient" do
    it "adds an ingredient to the ingredients array" do
      ...
      expect(dessert.ingredients).to eq(["milk", "eggs", "flour"])
    end
  end
  ...
  describe "#mix!" do
    it "shuffles the ingredient array" do
      ...
      expect(dessert.ingredients).not_to eq(["milk", "eggs", "flour"])
    end
  end
  ...
  describe "#serve" do
    it "contains the titleized version of the chef's name" do
      allow(chef).to receive(:titleize).and_return("Chef Chloe the Great Baker")
      expect(dessert.serve).to eq("Chef Chloe the Great Baker has made 5 puddings!")
    end
  end
